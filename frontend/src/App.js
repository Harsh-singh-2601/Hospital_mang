import React, { useEffect, useState } from "react"; // useState helps to change the values of state
...
export default function App() {
  ...
  useEffect(() => { // this allows to re render the things which are effected when there are any changes in userSession
    fetch("http://localhost:3001/userInSession")
      .then(res => res.json())
      .then(res => {
        setUserSession(res); // inbuilt react function that updates the usersession which was intially null
      })
      .catch(error => {
